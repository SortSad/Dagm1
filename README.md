...
local Des = {}
if makefolder and not isfile("negga Hub") then
    makefolder("negga Hub")
end 

local Settings = nil 

local Pcall = pcall(function()
    if isfile("negga Hub//" .. Name) then
        readfile("negga Hub//" .. Name)
    else 
        writefile("negga Hub//" .. Name, HttpService:JSONEncode(Des))
    end 
end)

if isfile("negga Hub//" .. Name) and readfile("negga Hub//" .. Name) then
    Settings = HttpService:JSONDecode(readfile("negga Hub//" .. Name))
end 

local Save = function()
    pcall(function()
        writefile("negga Hub//" .. Name, HttpService:JSONEncode(Settings))
    end)
end 
...
Section3:CreateLabel("Credits wirun")
Section3:CreateLabel("Credits Supa")
Section3:CreateLabel("Credits พ่อมึง")
